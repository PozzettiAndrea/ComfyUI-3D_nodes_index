...
This ComfyUI custom node enhances the standard CLIP text encoding functionality by integrating ethnicity, hairstyle, and expression selection into the positive prompt. Designed to work seamlessly with ComfyUI, this node allows you to influence the conditioning process by dynamically appending descriptive modifiers. Users can choose a specific ethnicity, hairstyle, or expression, or opt for a "random" selection that picks an option from a predefined CSV list.

![Example Interface](/img/img1.jpg)

## Features

- **Dynamic CSV Loading:**  
  The node reads options from CSV files (`ethnicity.csv`, `hairstyle.csv`, and `expression.csv`).  
...
- **Random Capability:**  
  If the "random" option is selected, the node picks a random ethnicity, hairstyle, or expression (excluding duplicates in the CSV) at runtime, adding variety to the prompt.

- **Toggle Functionality:**  
  Ethnicity, hairstyle, and expression modifications can be enabled or disabled independently using toggle inputs (`ethnicity_toggle`, `hairstyle_toggle`, and `expression_toggle`).

- **Customized Prompt Modification:**  
  The node preprocesses the positive text prompt by prepending the chosen ethnicity (e.g., "Asian person,"), hairstyle (e.g., "curly hair,"), and expression (e.g., "happy,") descriptors before encoding. This modification directly influences the conditioning outputs generated by the CLIP model.
...
1. **Required Inputs:**
   ...
   - `expression_toggle`: ("on" or "off") to enable expression inclusion.
...
2. **Optional Inputs:**
   ...
   - `expression`: Dropdown selector populated with sorted expression options; default is "random".
...
- **Prompt Modification:**  
  When enabled, the node modifies the positive prompt by appending the selected ethnicity, hairstyle, and/or expression descriptors. For example, if toggled on and ethnicity is set to "random", a random ethnicity is chosen from the CSV (excluding "random") and prepended to the prompt.
...
- Callbacks update application settings based on user selection for ethnicity, hairstyle, and expression.
...
## Extension Integration (JavaScript)
Example workflow for SDXL in the /workflow folder