...
이 노드는 입력 이미지에 2D 푸리에 변환(FFT)을 적용하여 진폭 스펙트럼과 위상 스펙트럼을 시각화합니다. 영상 처리, 이미지 분석 또는 예술적 효과를 위해 사용할 수 있습니다.
...
4. 두 개의 출력 핀(진폭 스펙트럼, 위상 스펙트럼)을 각각 원하는 노드에 연결합니다.

## 입력 및 출력

### 입력
- **image**: 변환할 이미지
- **amplitude**: 진폭 스펙트럼 시각화의 강도 (기본값: 20, 범위: 1-50)

### 출력
- **amplitude_spectrum**: 주파수 진폭 스펙트럼 시각화 결과 (로그 스케일)
- **phase_spectrum**: 주파수 위상 스펙트럼 시각화 결과 (-π ~ π → 0~1 정규화)
...
4. 진폭 스펙트럼: 주파수 진폭의 로그 스케일 값을 계산하여 시각화
5. 위상 스펙트럼: `np.angle()`로 위상값 추출 후 0~1로 정규화하여 시각화
6. 두 결과를 각각 정규화하여 출력
